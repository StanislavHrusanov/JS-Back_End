...
router.get('/profile', isLoggedIn, async (req, res) => {
    ...
    const myPosts = userDetails.myPublications.map(p => p.title).join(', ');
    const sharedPosts = userDetails.sharedPublications.map(p => p.title).join(', ');

    res.render('user/profile', { myPosts, sharedPosts });
});
